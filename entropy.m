...
metriks = evaluate_efficiency(a, a_prime);
cur_image = imread(file_path_a_prime);
compression_ratio = calculate_entropy(cur_image);
cur_entropy = 1/compression_ratio;
results = [results; {file_name, metriks(1), metriks(2), cur_entropy}];
end
end

function print_results_table(results)
fprintf('%-30s %-20s %-20s %-20s %-20s\n', 'Prime Image', 'DSF', 'GSF', 'Ds', 'Entropy');
fprintf('%-30s %-20s %-20s %-20s %-20s\n', '-----------', '----------', '----------', '----------', '----------');

for i = 1:length(results)
fprintf('%-30s %-20.4f %-20.4f %-20.4f %-20.4f\n', results{i, 1}, results{i, 2}, results{i, 3}, results{i, 2} * results{i, 3}, results{i, 4});
end
end
